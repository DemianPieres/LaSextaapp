...
export async function validateTicketByCode(token: string, codigoQR: string): Promise<AdminTicket> {
  // Codificar el código QR para la URL (manejar caracteres especiales)
  const encodedCode = encodeURIComponent(codigoQR.trim());
  const response = await apiFetch<{ ticket: AdminTicket }>(`/admin/tickets/validate/${encodedCode}`, {
    authToken: token,
    method: 'POST',
  });
  return response.ticket;
