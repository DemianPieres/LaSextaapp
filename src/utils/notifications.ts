...
export async function requestNotificationPermission(): Promise<boolean> {
  ...
  try {
    ...
    // Error al solicitar permisos - no crítico, retornar false
    return false;
  }
}
...
export async function scheduleLocalNotification(options: NotificationOptions): Promise<void> {
  ...
  try {
    ...
    if (!hasPermission) {
      // Permisos no otorgados - continuar sin notificación
      return;
    }
    ...
    // Error al programar notificación - no crítico, continuar sin notificar
  }
}
