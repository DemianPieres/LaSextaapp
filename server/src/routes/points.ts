...
export type RedeemCode = {
  ...
  fechaCreacion: Date;
  fechaExpiracion: Date | null; // null significa sin vencimiento
  fechaUso?: Date;
};
...
pointsRouter.post('/generate-redeem-code', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!reward) {
      res.status(404).json({ message: 'Premio no encontrado o no está habilitado.' });
      return;
    }

    const puntosACanjear = reward.puntosRequeridos;
    ...
    // Crear nuevo código de canje (sin fecha de vencimiento)
    const now = new Date();
    ...
    const redeemCode: RedeemCode = {
      ...
      estado: 'pendiente',
      fechaCreacion: now,
      fechaExpiracion: null, // Sin fecha de vencimiento
    };
    ...
    res.status(201).json({
      ...
      rewardId: reward._id.toHexString(),
      fechaExpiracion: null, // Sin fecha de vencimiento
    });
  } catch (error) {
    next(error);
...
adminPointsRouter.post('/add', async (req: AdminRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const rawResult = await db.collection(COLLECTION_USERS).findOneAndUpdate(
...
adminPointsRouter.post('/validate-redeem', async (req: AdminRequest, res: Response, next: NextFunction) => {
  try {
    ...
    // Verificar si expiró (solo si tiene fecha de vencimiento)
    if (redeemCode.fechaExpiracion && new Date() > redeemCode.fechaExpiracion) {
    ...
    const transaction: PointsTransaction = {
      ...
      descripcion: rewardInfo ? `Canje: ${rewardInfo.nombre}` : `Canje de ${redeemCode.puntosACanjear} puntos`,
      fecha: new Date(),
      procesadoPor: adminId,
    };
